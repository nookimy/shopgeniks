...
var gulp = require('gulp'),
    ...
    basePath = {
        ...
        title: 'Бытовая химия Ника', // Название сайта
        protocol: 'https://',
        domain: 'shopgeniks.ru' // URL сайта, без протокола
    },
    path = {
        // Пути до исходников
        src: {
...
if (basePath.srcCms) {
    ...
    if (basePath.templateName) {
        ...
        modules.forEach (function (module) {
            ...
            path.src.modulesWatch.push(basePath.srcCms + '/' + basePath.templateName + '/' + basePath.modulesFolder + '/' + module + '/js/*.js');
            path.src.modulesWatch.push(basePath.srcCms + '/' + basePath.templateName + '/' + basePath.modulesFolder + '/' + module + '/css/*.css');
...
if (basePath.srcCms && basePath.templateName) {
    gulp.task('copy-php-to-custom', function () {
            return gulp.src(basePath.srcCms + '/' + basePath.templateName + '/**/*.{php,css,js}')
...
if (basePath.srcCms && basePath.templateName) {
    gulp.task('watch-cms', function () {
        ...
        var targets = [basePath.srcCms + '/' + basePath.templateName + '/themes/*.php',
            basePath.srcCms + '/' + basePath.templateName + '/themes/blocks/*.php',
            basePath.srcCms + '/' + basePath.templateName + '/themes/functions/*.php'];
        targets = targets.concat(path.src.modulesWatch);

        gulp.watch(targets, gulp.series('copy-php-to-custom'));
    });
}
