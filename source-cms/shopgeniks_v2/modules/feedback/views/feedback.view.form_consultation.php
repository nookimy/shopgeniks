...
echo '
<form method="POST" enctype="multipart/form-data"  action="" class="feedback request__form request__form--consultation ajax">
...
if(! empty($result["rows"]))
{
	foreach ($result["rows"] as $row) //вывод полей из конструктора форм
	{
		...
		switch ($row["type"])
		{
			...
			case 'text':
				echo '<input class="feedback__input feedback__input--consultation" type="text" name="p'.$row["id"].'" value="" placeholder="'.$row["name"].'">
				<label class="visually-hidden" for="p'.$row["id"].'">'.$row["name"].'</label>';
				break;
			...
			case "phone":
				echo '<input class="feedback__input feedback__input--consultation" type="tel" name="p'.$row["id"].'" value="" placeholder="+ 7 ( _ _ _ ) _ _ _ - _ _ - _ _">
				<label class="visually-hidden" for="p'.$row["id"].'">'.$row["name"].'</label>';
				break;
...
//Кнопка Отправить
echo '<input type="submit" value="'.$this->diafan->_('Отправить', false).'" class="btn feedback__btn">';
