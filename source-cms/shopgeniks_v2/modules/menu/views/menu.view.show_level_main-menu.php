...
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	...
	if ($row["children"])
	{
		...
		echo '<li class="main-menu__item main-menu__item--header">';
	}
	else
	{
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a class="main-menu__link'.($row["children"]?" main-menu__link--header":"").'" href="'.$row["othurl"].'"'.$row["attributes"].'>';
		}
		else
		{
			echo '<a class="main-menu__link'.($row["children"]?" main-menu__link--header":"").'" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
		}
	}
    ...
    if ($result["show_all_level"] || $row["active_child"] || $row["active"])
    {
        ...
        if (empty($result['attributes']['count_level']) || $result['attributes']['count_level'] >= $menu_data["level"])
            echo $this->get('show_level_2_main-menu', 'menu', $menu_data);  //ниже функция, которая выводит в цикле пункты второго уровня вложенности.
    }
