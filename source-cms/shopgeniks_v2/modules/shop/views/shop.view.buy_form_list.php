...
if ($result["row"]["price_arr"]) {
	foreach ($result["row"]["price_arr"] as $price) {
        ...
        if(! empty($price["old_price"])) {
            echo '    <li class="prices__item prices__item--old">'.$price["old_price"].' '.$result["result"]["currency"].'</li>';
        }
...
echo '<div class="js_shop_buy shop_buy to-cart">';
	...
	{
	    echo '
              <input class="card__buy-input js-card__buy-input" type="button" action="buy">
              <div class="card__btn js-card__btn">
                  <svg class="card__bag-icon" width="17" height="21">
